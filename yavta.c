...
struct device
{
	...
	MMAL_COMPONENT_T *iencoder;
	MMAL_POOL_T *iencode_pool;
	...
	VCOS_THREAD_T save_image_thread;
	VCOS_THREAD_T save_thread;
	MMAL_QUEUE_T *save_queue;
	MMAL_QUEUE_T *save_image_queue;
	int thread_quit;
	FILE *h264_fd;
	FILE *image_fd;
	FILE *pts_fd;
...
static void list_formats(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(pixel_formats); i++)
		fprintf(stderr, "%s (\"%c%c%c%c\", %u planes)\n",
...
static int video_querycap(struct device *dev, unsigned int *capabilities)
{
	...
	fprintf(stderr, "Device `%s' on `%s' (driver '%s') is a video %s (%s mplanes) device.\n",
			 ...
			 struct v4l2_queryctrl *query)
{
	...
	if (ret < 0 && errno != EINVAL)
		fprintf(stderr, "unable to query control 0x%8.8x: %s (%d).\n",
			    ...
			    unsigned int flags)
{
	...
	if (video_is_mplane(dev)) {
		...
		fprintf(stderr, "stride is %d\n",stride);
		if (!stride)
			stride = ((w+31) &~31)*format_bpp(format);
		fprintf(stderr, "stride is now %d\n",stride);
...
static int buffer_export(int v4l2fd, enum v4l2_buf_type bt, int index, int *dmafd, unsigned int *vcsm_hdl)
{
	...
	{
		fprintf(stderr, "Failed to EXPBUF\n");
		return -1;
	}
	*dmafd = expbuf.fd;

	fprintf(stderr, "Importing DMABUF %d into VCSM...\n", expbuf.fd);
	vcsm_handle = vcsm_import_dmabuf(expbuf.fd, "V4L2 buf");
	if (vcsm_handle)
		fprintf(stderr, "...done. vcsm_handle %u\n", vcsm_handle);
	else
	...
	unsigned int offset, unsigned int padding)
{
	...
	fprintf(stderr, "%u buffers requested, V4L2 returned %u bufs.\n", nbufs, rb.count);
	...
	for (i = 0; i < rb.count; ++i) {
		...
		else
		{
			...
			{
				fprintf(stderr, "Some buffer exported whilst others not. HELP!\n");
		...
		if (dev->mmal_pool) {
			...
			if (!mmal_buf) {
				fprintf(stderr, "Failed to get a buffer from the pool. Queue length %d\n", mmal_queue_length(dev->mmal_pool->queue));
				return -1;
			}
			...
			fprintf(stderr, "Linking V4L2 buffer index %d ptr %p to MMAL header %p. mmal->data 0x%X\n",
...
static int video_print_control(struct device *dev, unsigned int id, bool full)
{
	...
	if (full)
		...
		fprintf(stderr, "control 0x%08x current %s.\n", query.id, current);
...
static void * save_thread(void *arg)
{
	...
	while (!dev->thread_quit)
	{
		...
		{
			fprintf(stderr, "No file to save to\n");
		}
		if (dev->pts_fd &&
		...
		{
			fprintf(stderr, "mmal_port_send_buffer failed on buffer %p, status %d", buffer, status);
		}
	}
	return NULL;
}
...
static void image_encoder_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
	...
	if (port->is_enabled)
		mmal_queue_put(dev->save_image_queue, buffer);
	else
		mmal_buffer_header_release(buffer);
}
...
{
	//fprintf(stderr, "Buffer %p from isp, filled %d, timestamp %llu, flags %04X\n", buffer, buffer->length, buffer->pts, buffer->flags);
	//vcos_log_error("File handle: %p", port->userdata);
	struct device *dev = (struct device*)port->userdata;
	...
	if (dev->iencoder)
	{
		MMAL_BUFFER_HEADER_T *out = mmal_queue_get(dev->iencode_pool->queue);
		if (out)
		{
			mmal_buffer_header_replicate(out, buffer);
			mmal_port_send_buffer(dev->iencoder->input[0], out);
		}
	}
...
{
	//fprintf(stderr, "Buffer %p returned from %s, filled %d, timestamp %llu, flags %04X\n", buffer, port->name, buffer->length, buffer->pts, buffer->flags);
	//vcos_log_error("File handle: %p", port->userdata);
	struct device *dev = (struct device*)port->userdata;
...
static void dump_port_format(MMAL_ES_FORMAT_T *format)
{
   ...
   fprintf(stderr, "type: %s, fourcc: %4.4s", name_type, (char *)&format->encoding);
   fprintf(stderr, " bitrate: %i, framed: %i", format->bitrate,
...
void mmal_log_dump_port(MMAL_PORT_T *port)
{
   ...
   fprintf(stderr, "%s(%p)", port->name, port);
...
static int setup_mmal(struct device *dev, int nbufs, int do_encode, const char *filename)
{
	...
	struct v4l2_format fmt;
	int ret;
	MMAL_PORT_T *isp_output, *encoder_input = NULL, *encoder_output = NULL, *image_encoder_input = NULL, *image_encoder_output = NULL;
	...
	if(status != MMAL_SUCCESS)
	{
		fprintf(stderr, "Failed to create isp\n");
		return -1;
	}
	...
	if (do_encode)
	{
		...
		if(status != MMAL_SUCCESS)
		{
			fprintf(stderr, "Failed to create video encoder\n");
			return -1;
		}
	...
	if (1)
	{
		...
		if(status != MMAL_SUCCESS)
		{
			fprintf(stderr, "Failed to create render component\n");
			return -1;
		}
		
		//Create the image encoder component
		status = mmal_component_create("vc.ril.image_encode", &dev->iencoder);
		if(status != MMAL_SUCCESS)
		{
			fprintf(stderr, "Failed to create image encoder\n");
		} else
		{
			fprintf(stderr, "Created image encoder\n");
	...
	if (mmal_stride != fmt.fmt.pix.bytesperline) {
		if (video_set_format(dev, fmt.fmt.pix.width, fmt.fmt.pix.height, fmt.fmt.pix.pixelformat, mmal_stride,
			...
			fprintf(stderr, "Failed to adjust stride\n");
		else
	...
	fprintf(stderr, "Created pool of length %d, size %d\n", nbufs, 0);
	...
	if (status != MMAL_SUCCESS)
	{
		fprintf(stderr, "ISP o/p commit failed\n");
		return -1;
	}
	...
	if (dev->iencoder)
	{
	   ...
	   if (image_encoder_output->buffer_num < image_encoder_output->buffer_num_min)
		  image_encoder_output->buffer_num = image_encoder_output->buffer_num_min;

	   // Commit the port changes to the output port
	   status = mmal_port_format_commit(image_encoder_output);
	   ...
	   // Set the JPEG quality level - always set to 100 for least amount of image detail loss
	   status = mmal_port_parameter_set_uint32(image_encoder_output, MMAL_PARAMETER_JPEG_Q_FACTOR, 100);
	   ...
	   if (status != MMAL_SUCCESS)
	   {
		  fprintf(stderr, "Unable to set JPEG restart interval\n");
		  return -1;
	   }
		...
		status = mmal_port_parameter_set_boolean(image_encoder_output, MMAL_PARAMETER_ZERO_COPY, MMAL_TRUE);
		status += mmal_port_parameter_set_boolean(image_encoder_input, MMAL_PARAMETER_ZERO_COPY, MMAL_TRUE);
		if(status != MMAL_SUCCESS)
		{
			fprintf(stderr, "Failed to set zero copy\n");
			return -1;
		}
	...
	if (dev->encoder)
	{
		...
		if (status != MMAL_SUCCESS)
		{
			fprintf(stderr, "Unable to set format on encoder output port\n");
		}
	...
	if (dev->iencoder)
	{
		status = mmal_port_enable(image_encoder_input, render_encoder_input_callback);
		if (status != MMAL_SUCCESS)
			return -1;
			
		fprintf(stderr, "Create pool of %d buffers of size %d for image encode ip\n", image_encoder_input->buffer_num, 0);
		dev->iencode_pool = mmal_port_pool_create(image_encoder_input, isp_output->buffer_num, isp_output->buffer_size);
		if(!dev->iencode_pool)
		{
			fprintf(stderr, "Failed to create image encoder ip pool\n");
			return -1;
		}
	}
	...
	fprintf(stderr, "Create pool of %d buffers of size %d for encode/render\n", isp_output->buffer_num, isp_output->buffer_size);
	...
	// setup image encoder buffers and port userdata struct
	if (dev->iencoder)
	{
		image_encoder_output->userdata = (void*)dev;
		
		fprintf(stderr, "Create image encoder output pool of %d buffers of size %d\n", image_encoder_output->buffer_num, image_encoder_output->buffer_size);
		dev->ioutput_pool = mmal_port_pool_create(image_encoder_output, image_encoder_output->buffer_num, image_encoder_output->buffer_size);
		if(!dev->ioutput_pool)
		{
		...
		if(status != MMAL_SUCCESS)
		{
			fprintf(stderr, "Failed to enable image encoder output port\n");
			return -1;
		}

		unsigned int i;
		for(i=0; i<image_encoder_output->buffer_num; i++)
		{
			MMAL_BUFFER_HEADER_T *buffer = mmal_queue_get(dev->ioutput_pool->queue);

			if (!buffer)
			{
				fprintf(stderr, "Where'd my buffer go?!\n");
				return -1;
			}
			status = mmal_port_send_buffer(image_encoder_output, buffer);
			if(status != MMAL_SUCCESS)
			{
				fprintf(stderr, "mmal_port_send_buffer to image encoder failed on buffer %p, status %d\n", buffer, status);
				return -1;
			}
	...
	if (dev->encoder)
	{
		...
		if(status != MMAL_SUCCESS)
		{
			fprintf(stderr, "Failed to enable port\n");
			return -1;
		}
		...
		for(i=0; i<encoder_output->buffer_num; i++)
		{
			...
			if(status != MMAL_SUCCESS)
			{
				fprintf(stderr, "mmal_port_send_buffer failed on buffer %p, status %d\n", buffer, status);
				return -1;
			}
			fprintf(stderr, "Sent buffer %p\n", buffer);
		}
...
static int enable_mmal_ip(struct device *dev)
{
	...
	if (status != MMAL_SUCCESS)
	{
		fprintf(stderr, "ISP input enable failed\n");
		return -1;
	}
	return 0;
}
	...
	int do_requeue_last, int do_queue_late, enum buffer_fill_mode fill)
{
	...
	if (ts.tv_sec == start.tv_sec && ts.tv_nsec == start.tv_nsec) {
		fprintf(stderr, "Captured %u frames (%u bytes) 0 seconds\n", i, size);
		goto done;
	}
	...
	fprintf(stderr, "Total number of frames dropped %d\n", dropped_frames);
done:
...
static void usage(const char *argv0)
{
	...
	fprintf(stderr, "				use -f help to list the supported formats\n");
	fprintf(stderr, "-E, --encode-to [file]		Set filename to write to. Default of file.h264.\n");
	...
	fprintf(stderr, "-T, --dv-timings		Query and set the DV timings\n");
	...
	fprintf(stderr, "    --buffer-prefix		Write portions of buffer before data_offset\n");
	fprintf(stderr, "    --buffer-size		Buffer size in bytes\n");
	...
	fprintf(stderr, "    --log-status		Log device status\n");
	fprintf(stderr, "    --no-query			Don't query capabilities on open\n");
	fprintf(stderr, "    --offset			User pointer buffer offset from page start\n");
	fprintf(stderr, "    --premultiplied		Color components are premultiplied by alpha value\n");
	fprintf(stderr, "    --queue-late		Queue buffers after streamon, not before\n");
	...
	fprintf(stderr, "-m  --mmal			Enable MMAL rendering of images\n");
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "B:c::Cd:E:f:F::hi:Ilmn:pq:r:R::s:t:Tuw:", opts, NULL)) != -1) {
		...
		case 'd':
			...
			break;
		case 'E':
			fprintf(stderr, "We're encoding to %s\n", optarg);
			do_encode = 1;
			if (optarg)
